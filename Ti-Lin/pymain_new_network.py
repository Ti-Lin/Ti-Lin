...
#Runs Ti-Lin with specified parameters
...
#Runs all Fast-Lin and Ti-Lin variations
def run_all_relu(layers, file_name, mlp_file_name, cifar = False, num_image=10, flfull = False, nonada = False):
    ...
    for norm in ['i', '2', '1']:
        LBss = []
        timess = []
        if nonada: #Run non adaptive Ti-Lin bounds
            LB, time = run_cnn(file_name, num_image, norm, cifar=cifar)
            printlog("Ti-Lin-relu")
            if filters:
        ...
        printlog("Ti-Lin-Ada, ReLU activation")
        if filters:
            printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,len(layers)+1,num_image,norm,filters,kernel_size))
        else:
            printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random".format(file_name,len(layers)+1,num_image,norm))
...
#Runs all general activation function versions of Ti-Lin
# 2022/10/29 added new params: tiny_imagenet relative to def run_cnn(tinyimagenet, ...)
    ...
    for norm in ['i', '2', '1']:
        ...
        if not onlyrelu:
            if not skipsigmoid:
                ...
                printlog("Ti-Lin-Ada, Sigmoid activation")
                if filters:
                    printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
                else:
                    printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
            ...
            printlog("Ti-Lin-Ada, Tanh activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
                printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
            ...
            printlog("Ti-Lin-Ada, Arctan activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
                printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
...
if __name__ == '__main__':
    ...
    if table == 3:
        ...
        #LB.append(LBs)                                                                
        #time.append(times)                                                            
        #LBs, times = run_all_general('../models/mnist_cnn_8layer', ada=True, onlyrelu=True, core=False, no_activate=False)
        #LB.append(LBs)
        #time.append(times)
        #LBs, times = run_all_general('../models/cifa_cnn_6layer', ada=True, onlyrelu=True, core=False, cifar=True, no_activate=False)
        #LB.append(LBs)
        #time.append(times)
        #LBs, times = run_all_general('../models/cifa_cnn_7layer', ada=True, onlyrelu=True, core=False, cifar=True, no_activate=False)
        #LB.append(LBs)
        #time.append(times)
        #LBs, times = run_all_general('../models/cifa_cnn_8layer', ada=True, onlyrelu=True, core=False, cifar=True, no_activate=False)
        #LB.append(LBs)
        #time.append(times)
	# new experiments
        ...
        LB.append(LBs)
        time.append(times)
        #LBs, times = run_all_general('../models/tinyimage_cnn_7layer', ada=True, onlyrelu=True, core=False, cifar=False, no_activate=False, tiny_imagenet=True)
        #LB.append(LBs)
        #time.append(times)
        #LBs, times = run_all_general('../models/tinyimage_cnn_8layer', ada=True, onlyrelu=True, core=False, cifar=False, no_activate=False, tiny_imagenet=True)
        #LB.append(LBs)
        #time.append(times)
    ...
    if table == 1:
        ...
        LBs, times = run_all_general('../models/mnist_cnn_4layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''
        ...
        '''
        '''
        LBs, times = run_all_general('./models/mnist_cnn_2layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/mnist_cnn_3layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/mnist_cnn_4layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/mnist_cnn_5layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/mnist_cnn_6layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/mnist_cnn_7layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/mnist_cnn_8layer_5_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        
        LBs, times = run_all_general('./models/mnist_cnn_8layer_10_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''
        LBs, times = run_all_general('./models/mnist_cnn_8layer_20_3', core=False,ada=False,cifar=False, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''


        LBs, times = run_all_general('./models/cifar_cnn_5layer_5_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/cifar_cnn_6layer_5_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/cifar_cnn_7layer_5_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''
        LBs, times = run_all_general('./models/cifar_cnn_8layer_5_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/cifar_cnn_5layer_10_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/cifar_cnn_5layer_20_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/cifar_cnn_7layer_10_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('./models/cifar_cnn_7layer_20_3', core=False,ada=False,cifar=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
