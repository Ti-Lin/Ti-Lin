...
import time
from activations import relu_linear_bounds, ada_linear_bounds, atan_linear_bounds, sigmoid_linear_bounds, tanh_linear_bounds,atan_linear_bounds_last,sigmoid_linear_bounds_last,tanh_linear_bounds_last
linear_bounds = None
linear_bounds_last = None
...
basic_block_2_cache = {}
def UL_basic_block_2_bound_last(A, B, pad, stride, W1, W2, W3, b1, b2, b3, pad1, pad2, pad3, stride1, stride2, stride3, upper=True):
    LB, UB = basic_block_2_cache[np.sum(W1)]
    A1, B1 = UL_conv_bound(A, B, np.asarray(pad), np.asarray(stride), np.asarray(UB.shape), W2, b2, np.asarray(pad2), np.asarray(stride2), np.asarray(UB.shape))
    inter_pad = (stride2[0]*pad[0]+pad2[0], stride2[0]*pad[1]+pad2[1], stride2[1]*pad[2]+pad2[2], stride2[1]*pad[3]+pad2[3])
    inter_stride = (stride2[0]*stride[0], stride2[1]*stride[1])
    alpha_u, alpha_l, beta_u, beta_l = linear_bounds_last(LB, UB)
    if upper:
        A1, B1 = UL_relu_bound(A1, B1, np.asarray(inter_pad), np.asarray(inter_stride), alpha_u, alpha_l, beta_u, beta_l)
    else:
        A1, B1 = UL_relu_bound(A1, B1, np.asarray(inter_pad), np.asarray(inter_stride), alpha_l, alpha_u, beta_l, beta_u)
...
basic_block_cache = {}
def UL_basic_block_bound_last(A, B, pad, stride, W1, W2, b1, b2, pad1, pad2, stride1, stride2, upper=True):
    LB, UB = basic_block_cache[np.sum(W1)]
    A1, B1 = UL_conv_bound(A, B, np.asarray(pad), np.asarray(stride), np.asarray(UB.shape), W2, b2, np.asarray(pad2), np.asarray(stride2), np.asarray(UB.shape))
    inter_pad = (stride2[0]*pad[0]+pad2[0], stride2[0]*pad[1]+pad2[1], stride2[1]*pad[2]+pad2[2], stride2[1]*pad[3]+pad2[3])
    inter_stride = (stride2[0]*stride[0], stride2[1]*stride[1])
                ...
                for p in range(p_start, p_end):
                    for q in range(q_start, q_end):
                        for r in range(A.shape[5]):
                            #print('*******')
                            #print(A_plus[x,y,z,p,q,r]*alpha_u[p+stride[0]*x-pad[0],q+stride[1]*y-pad[2],r])
                            A_new[x,y,z,p,q,r] += A_plus[x,y,z,p,q,r]*alpha_u[p+stride[0]*x-pad[0],q+stride[1]*y-pad[2],r]
                            #except:
                            #    print('*******')
                            #    print((A_plus[x,y,z,p,q,r]*alpha_u[p+stride[0]*x-pad[0],q+stride[1]*y-pad[2],r]).shape)
                                #print(A_new[x,y,z,p,q,r].shape)
                                #print(A[x,y,z,p,q,r].shape)
...
def compute_bounds(weights, biases, out_shape, nlayer, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs,wholelayer, aus, als, bus, bls):
    if types[nlayer-1] == 'relu':
        #print('relu')
        return np.maximum(LBs[nlayer-1], 0), np.maximum(UBs[nlayer-1], 0), np.maximum(aus[nlayer-1], 0), np.maximum(als[nlayer-1],0), np.maximum(bus[nlayer-1],0), np.maximum(bls[nlayer-1],0)
        ...
        LB, UB, _, _, _, _ = compute_bounds(weights[:nlayer-1]+[W1], biases[:nlayer-1]+[b1], out_shape, nlayer, x0, eps, p_n, pads[:nlayer-1]+[pad1], strides[:nlayer-1]+[stride1], sizes, types[:nlayer-1]+['conv'], LBs, UBs,wholelayer, aus, als, bus, bls)
        ...
        pad = (0,0,0,0)
        stride = (1,1)
        if nlayer==wholelayer-1:
            A_u, B_u = UL_basic_block_2_bound_last(A_u, B_u, pad, stride, *weights[nlayer-1], *biases[nlayer-1], *pads[nlayer-1], *strides[nlayer-1], upper=True)
            A_l, B_l = UL_basic_block_2_bound_last(A_l, B_l, pad, stride, *weights[nlayer-1], *biases[nlayer-1], *pads[nlayer-1], *strides[nlayer-1], upper=False)
        else:
        ...
        stride = (1,1)
        if nlayer==wholelayer-1:
            A_u, B_u = UL_basic_block_bound_last(A_u, B_u, pad, stride, *weights[nlayer-1], *biases[nlayer-1], *pads[nlayer-1], *strides[nlayer-1], upper=True)
            A_l, B_l = UL_basic_block_bound_last(A_l, B_l, pad, stride, *weights[nlayer-1], *biases[nlayer-1], *pads[nlayer-1], *strides[nlayer-1], upper=False)
        else:
    ...
    for i in range(nlayer-2, -1, -1):
        if types[i] == 'conv':
            ...
            if i==wholelayer-2 :
                ...
                for j in range(10):
                    A_u = weights[i+1].reshape((1, 1, weights[i+1].shape[0], weights[i+1].shape[1], weights[i+1].shape[2], weights[i+1].shape[3]))*np.ones((out_shape[0], out_shape[1], weights[i+1].shape[0], weights[i+1].shape[1], weights[i+1].shape[2], weights[i+1].shape[3]), dtype=np.float32)
                    B_u = biases[i+1]*np.ones((out_shape[0], out_shape[1], out_shape[2]), dtype=np.float32)
                    A_l = A_u.copy()
                    B_l = B_u.copy()
                ...
                pad = pads[i]
                stride = strides[i]
                alpha_u, alpha_l, beta_u, beta_l = linear_bounds_last(LBs[i], UBs[i],aus[-1],als[-1],i)
            elif  i==wholelayer-3:
                alpha_u, alpha_l, beta_u, beta_l = linear_bounds(LBs[i], UBs[i])#初始值
                A_u,A_l=func(i,weights, biases, out_shape,  pads, strides,  LBs, UBs,aus,als,alpha_u,alpha_l)#得到梯度下降的目标函数
                alpha_u, alpha_l=dfunc(A_u,A_l,alpha_u,alpha_l)#梯度下降
                alpha_u, alpha_l,beta_u, beta_l= linear_bound_last_2_atan(alpha_u,alpha_l,LBs[i], UBs[i])
                A_u,A_l,B_u,B_l=func2(i,weights, biases, out_shape,  pads, strides,  LBs, UBs,aus,als,alpha_u,alpha_l,beta_u, beta_l)
    ...
    LUB, UUB = conv_bound_full(A_u, B_u, np.asarray(pad), np.asarray(stride), x0, eps, p_n)
    LLB, ULB = conv_bound_full(A_l, B_l, np.asarray(pad), np.asarray(stride), x0, eps, p_n)
    #print(A_u, A_l, B_u, B_l)
    return LLB, UUB, A_u, A_l, B_u, B_l
...
def find_output_bounds(weights, biases, shapes, pads, strides, sizes, types, x0, eps, p_n):
    LBs = [x0-eps]
    UBs = [x0+eps]
    aus, bus = [[x0+eps] for i in range(2)]
    als, bls = [[x0-eps] for i in range(2)]
    for i in range(1,len(weights)+1):
        wholelayer=len(weights)
        print('Layer ' + str(i))
        LB, UB, a_u, a_l, b_u, b_l= compute_bounds(weights, biases, shapes[i], i, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs,wholelayer, aus, als, bus, bls)
        UBs.append(UB)
        LBs.append(LB)
        aus.append(a_u)
        als.append(a_l)
...
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    global linear_bounds,linear_bounds_last
    if activation == 'relu':
        linear_bounds = relu_linear_bounds
        linear_bounds_last=relu_linear_bounds
    elif activation == 'ada':
        linear_bounds = ada_linear_bounds
        linear_bounds_last=ada_linear_bounds
    elif activation == 'sigmoid':
        linear_bounds = sigmoid_linear_bounds
        linear_bounds_last=sigmoid_linear_bounds_last
    elif activation == 'tanh':
        linear_bounds = tanh_linear_bounds
        linear_bounds_last=tanh_linear_bounds_last
    elif activation == 'arctan':
        linear_bounds = atan_linear_bounds
        linear_bounds_last=atan_linear_bounds_last
...
def linear_bound_last_2_atan(alpha_u,alpha_l,LBs, UBs):
