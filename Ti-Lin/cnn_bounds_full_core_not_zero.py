...
#Main function to find bounds at each layer
@njit
def compute_bounds0(weights, biases, out_shape, nlayer, x0, eps, p_n, strides, pads, LBs, UBs):
    pad = (0,0,0,0)
    stride = (1,1)
    modified_LBs = LBs + (np.ones(out_shape, dtype=np.float32),)
    modified_UBs = UBs + (np.ones(out_shape, dtype=np.float32),)
...
def find_output_bounds0(weights, biases, shapes, pads, strides, x0, eps, p_n):
    ...
    for i in range(2,len(weights)+1):
        #print('Layer ' + str(i))
        LB, _, _, UB = compute_bounds0(tuple(weights), tuple(biases), shapes[i], i, x0, eps, p_n, tuple(strides), tuple(pads), tuple(LBs), tuple(UBs))
        UBs.append(UB)
        LBs.append(LB)
        f_mid.append(LB)
...
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    for i in range(len(inputs)):
        ...
        # shapes.append((1,1,1))
        LB,UB,f_middle = find_output_bounds(weights, biases, shapes, model.pads, model.strides, inputs[i].astype(np.float32), 0, p_n)
