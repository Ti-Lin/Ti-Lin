...
@njit
def pool_linear_bounds(LB, UB, pad, stride, pool_size,f_middle):
    #print('*********')
    p_hl, p_hr, p_wl, p_wr = pad
    s_h, s_w = stride
    ...
    #assert LB.all()>=f_middle.all()
    #assert UB.all()<=f_middle.all()
    for x in range(alpha_u.shape[2]):
        for y in range(alpha_u.shape[3]):
            for r in range(alpha_u.shape[4]):
                ...
                cropped_mid = f_middle[s_h*x-p_hl:pool_size[0]+s_h*x-p_hl, s_w*y-p_wl:pool_size[1]+s_w*y-p_wl,r]
                ...
                if il==iu:
                    weights[il] = 1
                    intercept = l_s[il] # 这是个新变量，我不知道怎么定义
                elif il==ju:#,3index=iu,ju=il ,il=ju
                    weights[iu] = (u_s[iu]- l_s[il]) / (u_s[iu] - l_s[iu])
                    weights[ju] = 1
                    intercept = l_s[il]
                else:
                    weights[iu] = (u_s[iu] - u_s[ku]) / (u_s[iu] - l_s[iu])
                    weights[ju] = (u_s[ju] - u_s[ku]) / (u_s[ju] - l_s[ju])
                    intercept = u_s[ku]#).astype(np.float32)
...
@njit
def upper_bound_pool(A, B, pad, stride, pool_size, inner_pad, inner_stride, inner_shape, LB, UB,f_middle):
    ...
    A_plus = np.maximum(A, 0)
    A_minus = np.minimum(A, 0)
    alpha_u, alpha_l, beta_u, beta_l = pool_linear_bounds(LB, UB, inner_pad, inner_stride, pool_size,f_middle)
...
@njit
def lower_bound_pool(A, B, pad, stride, pool_size, inner_pad, inner_stride, inner_shape, LB, UB,f_middle):
    A_new = np.zeros((A.shape[0], A.shape[1], A.shape[2], inner_stride[0]*(A.shape[3]-1)+pool_size[0], inner_stride[1]*(A.shape[4]-1)+pool_size[1], A.shape[5]), dtype=np.float32)
    B_new = np.zeros(B.shape, dtype=np.float32)
    A_plus = np.maximum(A, 0)
    A_minus = np.minimum(A, 0)
...
@njit
def compute_bounds(weights, biases, out_shape, nlayer, x0, eps, p_n, strides, pads, LBs, UBs,f_middle):
    pad = (0,0,0,0)
    stride = (1,1)
    modified_LBs = LBs + (np.ones(out_shape, dtype=np.float32),)
    modified_UBs = UBs + (np.ones(out_shape, dtype=np.float32),)
    modified_f_middle = f_middle + (np.ones(out_shape, dtype=np.float32),)
    for i in range(nlayer-1, -1, -1):
        ...
        if not np.isnan(weights[i]).any(): #Conv
            if i == nlayer-1:
                ...
                A_u, B_u = upper_bound_conv(A_u, B_u, pad, stride, weights[i], biases[i], pads[i], strides[i], modified_UBs[i].shape, modified_LBs[i+1], modified_UBs[i+1], modified_f_middle[i+1])
                A_l, B_l = lower_bound_conv(A_l, B_l, pad, stride, weights[i], biases[i], pads[i], strides[i], modified_LBs[i].shape, modified_LBs[i+1], modified_UBs[i+1], modified_f_middle[i+1])
        else: #Pool
            if i == nlayer-1:
                ...
                A_l = A_u.copy()
                B_l = B_u.copy()
            A_u, B_u = upper_bound_pool(A_u, B_u, pad, stride, weights[i].shape[1:], pads[i], strides[i], modified_UBs[i].shape, np.maximum(modified_LBs[i],0), np.maximum(modified_UBs[i],0),np.maximum(modified_f_middle[i],0))
            A_l, B_l = lower_bound_pool(A_l, B_l, pad, stride, weights[i].shape[1:], pads[i], strides[i], modified_LBs[i].shape, np.maximum(modified_LBs[i],0), np.maximum(modified_UBs[i],0),np.maximum(modified_f_middle[i],0))
...
#Main function to find bounds at each layer
@njit
def compute_bounds0(weights, biases, out_shape, nlayer, x0, eps, p_n, strides, pads, LBs, UBs, f_middle):
    pad = (0,0,0,0)
    stride = (1,1)
    modified_LBs = LBs + (np.ones(out_shape, dtype=np.float32),)
    modified_UBs = UBs + (np.ones(out_shape, dtype=np.float32),)
    ...
    for i in range(nlayer-1, -1, -1):
        ...
        if not np.isnan(weights[i]).any(): #Conv
            ...
            if i == nlayer-1:
                ...
                A_l = A_u.copy()
                B_l = B_u.copy()
            A_u, B_u = upper_bound_pool(A_u, B_u, pad, stride, weights[i].shape[1:], pads[i], strides[i], modified_UBs[i].shape, np.maximum(modified_LBs[i],0), np.maximum(modified_UBs[i],0), np.maximum(modified_UBs[i],0))
            A_l, B_l = lower_bound_pool(A_l, B_l, pad, stride, weights[i].shape[1:], pads[i], strides[i], modified_LBs[i].shape, np.maximum(modified_LBs[i],0), np.maximum(modified_UBs[i],0),np.maximum(modified_UBs[i],0))
        pad = (strides[i][0]*pad[0]+pads[i][0], strides[i][0]*pad[1]+pads[i][1], strides[i][1]*pad[2]+pads[i][2], strides[i][1]*pad[3]+pads[i][3])
        stride = (strides[i][0]*stride[0], strides[i][1]*stride[1])
    LUB, UUB = conv_bound_full(A_u, B_u, pad, stride, x0, eps, p_n)
    LLB, ULB = conv_bound_full(A_l, B_l, pad, stride, x0, eps, p_n)
...
def find_output_bounds(weights, biases, shapes, pads, strides, x0, eps, p_n,f_middle):
    LB, UB = conv_bound(weights[0], biases[0], pads[0], strides[0], x0, eps, p_n)
    LBs = [x0-eps, LB]
    UBs = [x0+eps, UB]
    for i in range(2,len(weights)+1):
        #print('Layer ' + str(i))
        LB, _, _, UB = compute_bounds(tuple(weights), tuple(biases), shapes[i], i, x0, eps, p_n, tuple(strides), tuple(pads), tuple(LBs), tuple(UBs),tuple(f_middle))
        UBs.append(UB)
        LBs.append(LB)
    return LBs[-1], UBs[-1]

#Main function to find output bounds
def find_output_bounds0(weights, biases, shapes, pads, strides, x0, eps, p_n):
    LB, UB = conv_bound(weights[0], biases[0], pads[0], strides[0], x0, eps, p_n)
    LBs = [x0-eps, LB]
    UBs = [x0+eps, UB]
    f_mid=[x0+eps, UB]
    for i in range(2,len(weights)+1):
        #print('Layer ' + str(i))
        LB, _, _, UB = compute_bounds0(tuple(weights), tuple(biases), shapes[i], i, x0, eps, p_n, tuple(strides), tuple(pads), tuple(LBs), tuple(UBs), tuple(f_mid))
        UBs.append(UB)
        LBs.append(LB)
        f_mid.append(LB)
    return LBs[-1],UBs[-1],f_mid
...
#Main function to compute Tilin bound
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    for i in range(len(inputs)):
        ...
        biases = model.biases
        shapes = model.shapes
        # W, b, s = model.weights[-1], model.biases[
        # -1], model.shapes[-1]
        ...
        # shapes.append((1,1,1))
        LB,UB,f_middle = find_output_bounds0(weights, biases, shapes, model.pads, model.strides, inputs[i].astype(np.float32), 0, p_n)
        ...
        for j in range(steps):
            LB, UB = find_output_bounds(weights, biases, shapes, model.pads, model.strides, inputs[i].astype(np.float32), np.exp(log_eps), p_n,f_middle)
