...
Main Ti-Lin-R computation file for general networks
...
def compute_bounds0(weights, biases, out_shape, nlayer, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs):
    ...
    for i in range(nlayer-2, -1, -1):
        if types[i] == 'conv':
            ...
            #print('relu')
            alpha_u, alpha_l, beta_u, beta_l = linear_bounds(LBs[i], LBs[i])
            A_u, B_u = UL_relu_bound(A_u, B_u, np.asarray(pad), np.asarray(stride), alpha_u, alpha_l, beta_u, beta_l)
            A_l, B_l = UL_relu_bound(A_l, B_l, np.asarray(pad), np.asarray(stride), alpha_l, alpha_u, beta_l, beta_u)
        elif types[i] == 'basic_block_2':
            #print('basic block 2')
...
#Main function to compute Ti-Lin-R bound
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    for i in range(len(inputs)):
        print('--- Ti-Lin-R: Computing eps for input image ' + str(i)+ '---')
        ...
        print("[L1] method = Ti-Lin-R-{}, model = {}, image no = {}, true_id = {}, target_label = {}, true_label = {}, norm = {}, robustness = {:.5f}".format(activation,file_name, i, true_ids[i],target_label,predict_label,str_p_n,np.exp(log_eps_min)))
        summation += np.exp(log_eps_min)
    K.clear_session()
    
    eps_avg = summation/len(inputs)
    total_time = (time.time()-start_time)/len(inputs)
    print("[L0] method = Ti-Lin-R-{}, model = {}, total images = {}, norm = {}, avg robustness = {:.5f}, avg runtime = {:.2f}".format(activation,file_name,len(inputs),str_p_n,eps_avg,total_time))
