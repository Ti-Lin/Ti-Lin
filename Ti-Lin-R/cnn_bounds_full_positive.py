...
Main VeriNet computation file for general networks
...
import time
from activations import relu_linear_bounds, ada_linear_bounds, atan_linear_bounds_test, sigmoid_linear_bounds, tanh_linear_bounds,atan_linear_bounds,sigmoid,atan,tanh,sigmoid_linear_bounds_test,tanh_linear_bounds_test,ada_linear_bounds_test
linear_bounds = None
...
@njit
def UL_relu_bound(A, B, pad, stride, alpha_u, alpha_l, beta_u, beta_l,du,dl):
                ...
                for p in range(p_start, p_end):
                    for q in range(q_start, q_end):
                        for r in range(A.shape[5]):
                            ...
                            du_new[x,y,z] += A_plus[x,y,z,p,q,r]*du[p+stride[0]*x-pad[0],q+stride[1]*y-pad[2],r]
                            du_new[x,y,z] += A_minus[x,y,z,p,q,r]*dl[p+stride[0]*x-pad[0],q+stride[1]*y-pad[2],r]+B[x,y,z]
    return A_new, B_new,du_new


@njit
def UL_relu_bound0(A, B, pad, stride, alpha_u, alpha_l, beta_u, beta_l):
    A_new = np.zeros_like(A)
    A_plus = np.maximum(A, 0)
    A_minus = np.minimum(A, 0)
    B_new = B.copy()
...
def compute_bounds(weights, biases, out_shape, nlayer, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs,dus,dls,f_middle):
    # flag=0
    if types[nlayer-1] == 'relu':
        #print('relu')
        return act(LBs[nlayer-1]), act(UBs[nlayer-1]),0,0#放在relu后面一层的dus上
        ...
        # DU=f_middle[nlayer-1]
        # DL=f_middle[nlayer-1]
    ...
    for i in range(nlayer-2, -1, -1):
        if types[i] == 'conv':
            ...
            # fmiddleu=f_middle[i]+dus[i]#i是relu之前的函数值
            # fmiddlel=f_middle[i]+dls[i]#relu前一层的du

            # if flag==1:
            alpha_u, alpha_l, beta_u, beta_l= linear_bounds(LBs[i], UBs[i],dus[i],dls[i])
            A_u, B_u = UL_relu_bound0(A_u, B_u, np.asarray(pad), np.asarray(stride), alpha_u, alpha_l, beta_u, beta_l)
            A_l, B_l = UL_relu_bound0(A_l, B_l, np.asarray(pad), np.asarray(stride), alpha_l, alpha_u, beta_l, beta_u)
                # print(flag)
            # else:
            #     # if i != 1:
            #     #     print(f_middle[i].shape)
            #     #     print(dls[i].shape)

            #     alpha_u, alpha_l, beta_u, beta_l,du,dl = linear_bounds(LBs[i], UBs[i],dus[i],dus[i])
            #     # du=du-f_middle[i+1]#relu的值
            #     # dl=dl-f_middle[i+1]
            #     A_u, B_u,DU = UL_relu_bound(A_u, B_u, np.asarray(pad), np.asarray(stride), alpha_u, alpha_l, beta_u, beta_l,du,dl)
            #     A_l, B_l,DL = UL_relu_bound(A_l, B_l, np.asarray(pad), np.asarray(stride), alpha_l, alpha_u, beta_l, beta_u,dl,du)                      
            #     # flag=1
                # print(flag)
    ...
    LUB, DU = conv_bound_full(A_u, B_u, np.asarray(pad), np.asarray(stride), x0, 0, p_n)
    DL, ULB = conv_bound_full(A_l, B_l, np.asarray(pad), np.asarray(stride), x0, 0, p_n)

    return LLB,UUB,DU,DL
#Main function to find bounds at each layer
def compute_bounds0(weights, biases, out_shape, nlayer, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs):
    ...
    for i in range(nlayer-2, -1, -1):
        if types[i] == 'conv':
            ...
            #print('relu')
            alpha_u, alpha_l, beta_u, beta_l = linear_bounds(LBs[i], LBs[i],LBs[i], LBs[i])
            A_u, B_u = UL_relu_bound0(A_u, B_u, np.asarray(pad), np.asarray(stride), alpha_u, alpha_l, beta_u, beta_l)
            A_l, B_l = UL_relu_bound0(A_l, B_l, np.asarray(pad), np.asarray(stride), alpha_l, alpha_u, beta_l, beta_u)
...
def find_output_bounds(weights, biases, shapes, pads, strides, sizes, types, x0, eps, p_n,f_middle):
    ...
    for i in range(1,len(weights)+1):
        ...
        LB, UB,DU,DL = compute_bounds(weights, biases, shapes[i], i, x0, eps, p_n, pads, strides, sizes, types, LBs, UBs,dus,dls,f_middle)
        UBs.append(UB)
        LBs.append(LB)
        dus.append(DU)
        dls.append(DL)
...
#Main function to compute VeriNet bound
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    global linear_bounds,linear_bounds0,act
    if activation == 'relu':
        linear_bounds = relu_linear_bounds
        act=relu
    elif activation == 'ada':
        linear_bounds = ada_linear_bounds_test#ada_linear_bounds
        act=relu
    elif activation == 'sigmoid':
        linear_bounds = sigmoid_linear_bounds_test     
        act=sigmoid
    elif activation == 'tanh':
        linear_bounds = tanh_linear_bounds_test         
        act=tanh
    elif activation == 'arctan':
        linear_bounds = atan_linear_bounds_test        
        act=atan
    ...
    for i in range(len(inputs)):
        print('--- VeriNet: Computing eps for input image ' + str(i)+ '---')
        ...
        print("[L1] method = VeriNet-{}, model = {}, image no = {}, true_id = {}, target_label = {}, true_label = {}, norm = {}, robustness = {:.5f}".format(activation,file_name, i, true_ids[i],target_label,predict_label,str_p_n,np.exp(log_eps_min)))
        summation += np.exp(log_eps_min)
    K.clear_session()
    
    eps_avg = summation/len(inputs)
    total_time = (time.time()-start_time)/len(inputs)
    print("[L0] method = VeriNet-{}, model = {}, total images = {}, norm = {}, avg robustness = {:.5f}, avg runtime = {:.2f}".format(activation,file_name,len(inputs),str_p_n,eps_avg,total_time))
