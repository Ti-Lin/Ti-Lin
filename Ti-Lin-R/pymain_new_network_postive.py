...
from cnn_bounds_full_positive import run as run_cnn_full
...
#Runs Ti-Lin-R with specified parameters
...
#Runs all Fast-Lin and Ti-Lin-R variations
def run_all_relu(layers, file_name, mlp_file_name, cifar = False, num_image=10, flfull = False, nonada = False):
    ...
    for norm in ['i', '2', '1']:
        LBss = []
        timess = []
        if nonada: #Run non adaptive Ti-Lin-R bounds
            LB, time = run_cnn(file_name, num_image, norm, cifar=cifar)
            printlog("Ti-Lin-R-relu")
            if filters:
        ...
        printlog("Ti-Lin-R-Ada, ReLU activation")
        if filters:
            printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,len(layers)+1,num_image,norm,filters,kernel_size))
        else:
            printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random".format(file_name,len(layers)+1,num_image,norm))
...
#Runs all general activation function versions of Ti-Lin-R
    ...
    for norm in ['i','2', '1']:
        ...
        if not onlyrelu:
            if not skipsigmoid:
                ...
                printlog("Ti-Lin-R-Ada, Sigmoid activation")
                if filters:
                    printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
                else:
                    printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
            ...
            printlog("Ti-Lin-R-Ada, Tanh activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
                printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
            ...
            printlog("Ti-Lin-R-Ada, Arctan activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
                printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
...
if __name__ == '__main__':
    ...
    if table == 4:
        ...
        time.append(times)
        '''
        LBs, times = run_all_general('models/cifar_cnn_8layer_5_3', core=False,ada=True,cifar=True,skipsigmoid=True)
        LB.append(LBs)
        time.append(times)
        '''
