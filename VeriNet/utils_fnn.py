        ...
        if targeted:
            ...
            if random_and_least_likely:
                ...
                if target_classes is None:
                    ...
                    #print(data.test_data[start+i])
                    if not cifa:

                        original_predict = np.squeeze(predictor(data.test_data[start+i].transpose(2, 0, 1)))
                        #original_predict = np.squeeze(predictor(np.squeeze(data.test_data[start+i][np.newaxis,:],3)))#.transpose(2, 0, 1)))
                    else:
            ...
            for j in seq:
                ...
                if (j == np.argmax(data.test_labels[start+i])):
                    continue
                #inputs.append(np.squeeze(data.test_data[start+i]))#.transpose(2, 0, 1))
