...
#Prints to log file
def printlog(s):
    print(s, file=open("log_pymain_only_ti_"+timestr+".txt", "a"))
...
def run_all_general(file_name, num_image = 10, core=True, cifar=False, ada=True, onlyrelu=False, skipsigmoid=False):
    ...
    for norm in ['i', '2', '1']:
        ...
        '''
        if ada:
            LB, time = run_cnn(file_name, num_image, norm, core=core, activation = 'ada', cifar= cifar, tinyimagenet=True)
            printlog("CNN-Cert-Ada, ReLU activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
...
if __name__ == '__main__':
    ...
    if table == -2:
        # Table 5
        LBs, times = run_all_general('/root/new-Ti-Lin/models/tiny_cnn_7layer_7080.hdf5', onlyrelu=True, core=True)
        LB.append(LBs)
        time.append(times)
        '''
        ...
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifar_cnn_7layer', onlyrelu=True, core=True)
        LB.append(LBs)
        time.append(times)
        '''
    if table == 6:
    ...
    if table == -1:
        ...
        #LBs, times = run_all_general('models/mnist_cnn_8layer_5_3', core=False)
        #LB.append(LBs)
        LBs, times = run_all_general('models/mnist_cnn_lenet', core=False, skipsigmoid=True)
        LB.append(LBs)
