...
from cnn_bounds_full_postive import run as run_cnn_full
...
#Runs VeriNet with specified parameters
...
#Runs all Fast-Lin and VeriNet variations
def run_all_relu(layers, file_name, mlp_file_name, cifar = False, num_image=10, flfull = False, nonada = False):
    ...
    for norm in ['i', '2', '1']:
        LBss = []
        timess = []
        if nonada: #Run non adaptive VeriNet bounds
            LB, time = run_cnn(file_name, num_image, norm, cifar=cifar)
            printlog("VeriNet-relu")
            if filters:
        ...
        printlog("VeriNet-Ada, ReLU activation")
        if filters:
            printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,len(layers)+1,num_image,norm,filters,kernel_size))
        else:
            printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random".format(file_name,len(layers)+1,num_image,norm))
...
#Runs all general activation function versions of VeriNet
    ...
    for norm in ['i','1','2']:
        ...
        if not onlyrelu:
            if not skipsigmoid:
                ...
                printlog("VeriNet-Ada, Sigmoid activation")
                if filters:
                    printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
                else:
                    printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
            ...
            printlog("VeriNet-Ada, Tanh activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
                printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
            ...
            printlog("VeriNet-Ada, Arctan activation")
            if filters:
                printlog("model name = {0}, numlayer = {1}, numimage = {2}, norm = {3}, targettype = random, filters = {4}, kernel size = {5}".format(file_name,nlayer,num_image,norm,filters,kernel_size))
            else:
                printlog("model name = {0}, numimage = {1}, norm = {2}, targettype = random".format(file_name,num_image,norm))
...
if __name__ == '__main__':
    ...
    if table == -2:
        ...
        '''
        LBs, times = run_all_general('models/tiny_cnn_7layer', onlyrelu=True)#, core=True)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/mnist_cnn_7layer', onlyrelu=True, core=True)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/mnist_cnn_lenet', onlyrelu=True, core=True)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifar_cnn_7layer', onlyrelu=True, core=True, cifar = True)
        LB.append(LBs)
        time.append(times)
        '''
        '''
        LBs, times = run_all_general('models/mnist_cnn_6layer', ada=True, onlyrelu=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/mnist_cnn_7layer', ada=True, onlyrelu=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/mnist_cnn_8layer', ada=True, onlyrelu=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifa_cnn_6layer', ada=True, onlyrelu=True, cifar=True, core=False)
        LB.append(LBs)
        time.append(times)
        '''
        #LBs, times = run_all_general(/models/cifa_cnn_7layer', ada=True, onlyrelu=True, cifar=True, core=False)
        #LB.append(LBs)
        #time.append(times)
        '''
        LBs, times = run_all_general('models/cifa_cnn_8layer', ada=True, onlyrelu=True, cifar=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/mnist_cnn_6layer_pool', ada=False, onlyrelu=False, core=False)
        LB.append(LBs)
        time.append(times)
        
        LBs, times = run_all_general('models/mnist_cnn_8layer_pool', ada=False, onlyrelu=False, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifa_cnn_6layer_pool', ada=False, onlyrelu=False, cifar=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifa_cnn_7layer_pool', ada=False, onlyrelu=False, cifar=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifa_cnn_8layer_pool', ada=False, onlyrelu=False, cifar=True, core=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('models/cifa_cnn_7layer', ada=True, onlyrelu=True, cifar=True, core=False)
        LB.append(LBs)
        time.append(times)
        '''
        LBs, times = run_all_general('models/mnist_cnn_7layer_pool', ada=False, onlyrelu=False, core=False)
        LB.append(LBs)
        time.append(times)
    ...
    if table == 4:
        ...
        LBs, times = run_all_general('../models/mnist_cnn_4layer_5_3', core=False,ada=False,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('../models/mnist_cnn_5layer_5_3', core=False,ada=False,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('../models/mnist_cnn_6layer_5_3', core=False,ada=False,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('../models/mnist_cnn_7layer_5_3', core=False,ada=False,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''
        LBs, times = run_all_general('../models/mnist_cnn_8layer_5_3', core=False,ada=False,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''

        LBs, times = run_all_general('../models/cifar_cnn_5layer_5_3', core=False,ada=False,cifar=True,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('../models/cifar_cnn_6layer_5_3', core=False,ada=False,cifar=True,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        LBs, times = run_all_general('../models/cifar_cnn_7layer_5_3', core=False,ada=False,cifar=True,skipsigmoid=True, no_activate=False)
        LB.append(LBs)
        time.append(times)
        '''
        LBs, times = run_all_general('models/cifar_cnn_8layer_5_3', core=False,ada=False,cifar=True,skipsigmoid=True)
        LB.append(LBs)
        time.append(times)
        '''
